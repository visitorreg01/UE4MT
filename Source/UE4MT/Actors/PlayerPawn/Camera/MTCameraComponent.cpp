...
UMTCameraComponent::UMTCameraComponent(const FObjectInitializer& ObjectInitializer)
...
void UMTCameraComponent::OnZoomOut()
{
    SetZoomLevel(ZoomAlpha + 0.1f);
}

void UMTCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& OutResult)
{
...
void UMTCameraComponent::UpdateCameraMovement(const APlayerController* InPlayerController)
{
    ...
    if (LocalPlayer && LocalPlayer->ViewportClient && LocalPlayer->ViewportClient->Viewport)
    {
        ...
        if (GetPlayerController() != NULL)
        {
            ...
            {
                SpectatorCameraSpeed = GetDefault<UMTSpectatorPawnMovement>(SpectatorPawn->GetMovementComponent()->GetClass())->MaxSpeed;
            }
        }
        if (!bNoScrollZone)
        {
...
void UMTCameraComponent::MoveForward(float Val)
{
...
void UMTCameraComponent::MoveRight(float Val)
{
    APawn* OwnerPawn = GetOwnerPawn();
    if (OwnerPawn != NULL)
    {
...
void UMTCameraComponent::AddNoScrollZone(FBox InCoords)
{
    NoScrollZones.AddUnique(InCoords);
}

void UMTCameraComponent::ClampCameraLocation(const APlayerController* InPlayerController, FVector& OutCameraLocation)
{
...
void UMTCameraComponent::UpdateCameraBounds(const APlayerController* InPlayerController)
{
    ULocalPlayer* const LocalPlayer = Cast<ULocalPlayer>(InPlayerController->Player);
        ...
        if (MyGameState)
        {
            ...
            if (WorldBounds.GetSize() != FVector::ZeroVector)
            {
                ...
                const FVector CameraPlanePoints[3] = {
                    FMTHelpers::IntersectRayWithPlane(WorldBoundPoints[0], FrustumRays[0], CameraPlane)*MiniMapBoundsLimit,
                    FMTHelpers::IntersectRayWithPlane(WorldBoundPoints[1], FrustumRays[1], CameraPlane)*MiniMapBoundsLimit,
                    FMTHelpers::IntersectRayWithPlane(WorldBoundPoints[2], FrustumRays[2], CameraPlane)*MiniMapBoundsLimit
                };
...
APlayerController* UMTCameraComponent::GetPlayerController()
{
...
APawn* UMTCameraComponent::GetOwnerPawn()
{
    return Cast<APawn>(GetOwner());
}

void UMTCameraComponent::OnPinchStarted(const FVector2D& AnchorPosition1, const FVector2D& AnchorPosition2, float DownTime)
{
...
void UMTCameraComponent::OnPinchUpdate(UMTInput* InputHandler, const FVector2D& ScreenPosition1, const FVector2D& ScreenPosition2, float DownTime)
{
...
void UMTCameraComponent::SetCameraTarget(const FVector& CameraTarget)
{
...
void UMTCameraComponent::SetZoomLevel(float NewLevel)
{
    ZoomAlpha = FMath::Clamp(NewLevel, MinZoomLevel, MaxZoomLevel);
}

bool UMTCameraComponent::OnSwipeStarted(const FVector2D& SwipePosition)
{
    bool bResult = false;
...
bool UMTCameraComponent::OnSwipeUpdate(const FVector2D& SwipePosition)
{
    bool bResult = false;
    APlayerController* Controller = GetPlayerController();
...
bool UMTCameraComponent::OnSwipeReleased(const FVector2D& SwipePosition)
{
    bool bResult = false;
...
void UMTCameraComponent::EndSwipeNow()
{
    StartSwipeCoords.Set(0.0f, 0.0f, 0.0f);
}

bool UMTCameraComponent::AreCoordsInNoScrollZone(const FVector2D& SwipePosition)
{
    bool bResult = false;
