...
UMTInput::UMTInput(const FObjectInitializer& ObjectInitializer)
    : Super(ObjectInitializer)
    , PrevTouchState(0)
{
}

void UMTInput::UpdateDetection(float DeltaTime)
{
    UpdateGameKeys(DeltaTime);
    ProcessKeyStates(DeltaTime);
}

void UMTInput::ProcessKeyStates(float DeltaTime)
{
    ...
    // update states
    for (TMap<GameKeyTypeEnum, FSimpleKeyState>::TIterator It(KeyStateMap); It; ++It)
    {
...
void UMTInput::UpdateGameKeys(float DeltaTime)
{
    AMTSpectatorPlayerController* MyController = CastChecked<AMTSpectatorPlayerController>(GetOuter());
...
void UMTInput::DetectOnePointActions(bool bCurrentState, bool bPrevState, float DeltaTime, const FVector2D& CurrentPosition, FVector2D& AnchorPosition, float& DownTime)
{
    ...
    if (bCurrentState && !bTwoPointsTouch)
    {
        ...
        FSimpleKeyState& SwipeState = KeyStateMap.FindOrAdd(GameKeyTypeEnum::Swipe);
        if (SwipeState.bDown)
        {
        ...
        {
            FSimpleKeyState& HoldState = KeyStateMap.FindOrAdd(GameKeyTypeEnum::Hold);
    ...
    else
    {
        ...
        if (bPrevState)
        {
            ...
            {
                FSimpleKeyState& TapState = KeyStateMap.FindOrAdd(GameKeyTypeEnum::Tap);
...
void UMTInput::DetectTwoPointsActions(bool bCurrentState, bool bPrevState, float DeltaTime, const FVector2D& CurrentPosition1, const FVector2D& CurrentPosition2)
{
    ...
    if (bCurrentState)
    {
        ...
        if (!bPrevState)
        {
            ...
            {
                FSimpleKeyState& SwipeState = KeyStateMap.FindOrAdd(GameKeyTypeEnum::SwipeTwoPoints);
                SwipeState.Events[IE_Pressed]++;
            ...
            FSimpleKeyState& PinchState = KeyStateMap.FindOrAdd(GameKeyTypeEnum::Pinch);
...
FVector2D UMTInput::GetTouchAnchor(int32 i) const
{
